...
//			    errs()<<var.name<<" = ["<<x_lb<<", "<<x_ub<<"]\n";
	...
	~VarTable(){varVal.clear();alias.clear();storeMap.clear();exprMap.clear();x.clear();var_num=0;alloca_num=0;cfg=NULL;}
...
public:
    BoundedVerification(CFG* aut,int bound,vector<int> target,double pre, DebugInfo *dbg);
