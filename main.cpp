...
void opt(string name, int b, double p, int m, char* f, char* e, int o){
    ...
    string command = "opt -load buildCFG.so -load /usr/local/lib/libcapd.so -load /usr/local/lib/libibex.so"
    " -load /usr/local/lib/libdreal.so -load /usr/lib/libz3.so -load /usr/local/lib/libminisat.so"
