...
BoundedVerification::BoundedVerification(CFG* aut, int bound, vector<int> target, double pre, DebugInfo *dbg){
	this->cfg=aut;
	this->bound=bound;
	this->target=target;
	this->precision=pre;
...
void BoundedVerification::setOutMode(int output){
	this->outMode = output;
	this->verify.setOutMode(output);
}
